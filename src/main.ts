function piece(color: Color, profession: Profession, column: Column, row: Row, degree: Degree) {
    const TOP = {
    ...
    g.setAttributeNS('http://www.w3.org/2000/svg', 'transform',
        `translate(${LEFT * 17.55} ${TOP * 17.55}) rotate(${degree} ${rot_center.x} ${rot_center.y}) translate(${offsets.x} ${offsets.y})`)
    g.appendChild(piece_path(color, profession));
    return g;
}

function piece_path(color: Color, profession: Profession) {
...
function main_g(input: Array<[Color, Profession, Column, Row, Degree]>,
    o: {
        water_edge: boolean,
        coordinate: boolean,
        tamhue_background: boolean,
        tamnua_background: boolean,
        tamhue_slash: boolean,
        tamzo_color: "orange" | "white" | "green" | "blue" | null
    }) {
    const g = document.createElementNS('http://www.w3.org/2000/svg', "g");
    ...
    function square_inv(column: Column, row: Row, fill: string) {
        const LEFT = {
            P: 8, M: 7, C: 6, X: 5, Z: 4, T: 3, N: 2, L: 1, K: 0
        }[column];
    ...
    for (let i = 0; i < 9; i++) {
        const clms: ReadonlyArray<Column> = ['P', 'M', 'C', 'X', 'Z', 'T', 'N', 'L', 'K'];
        const column: Column = clms[i]
        for (let j = 0; j < 9; j++) {
            ...
            if (["ZI", "ZU", "NO", "TO", "XO", "CO", "ZY", "ZAI"].includes(`${column}${row}`)) {
                ...
                switch (o.tamzo_color) {
                    case "white": g.innerHTML += square_inv(column, row, "none"); break;
                    case "green": g.innerHTML += square_inv(column, row, "#005242"); break;
                    case "orange": g.innerHTML += square_inv(column, row, '#ffb380'); break;
                    case "blue": g.innerHTML += square_inv(column, row, '#87aade'); break;
                }
            } else {
...
function render() {
    const input: Array<[Color, Profession, Column, Row, Degree]> = (document.getElementById("board_state")! as HTMLInputElement).value
        .split("\n")
        ...
        .flatMap(line => {
            ...
            let msg = [];
            if (!isColor(color)) { msg.push(`invalid color ${color}`); }
            if (!isProfession(profession)) { msg.push(`invalid profession ${profession}`); }
            if (!isColumn(column)) { msg.push(`invalid column ${column}`); }
            if (!isRow(row)) { msg.push(`invalid row ${row}`); }
            if (Number.isNaN(degree_num)) { msg.push(`invalid degree ${degree_str}`); }
            alert(`${msg.join(", ")} detected. Skipping.`)
            return [];
        });
    const g = main_g(input, {
        water_edge: (document.getElementById("water_edge")! as HTMLInputElement).checked,
        coordinate: (document.getElementById("coordinate")! as HTMLInputElement).checked,
        tamhue_background: (document.getElementById("tamhue_background")! as HTMLInputElement).checked,
        tamnua_background: (document.getElementById("tamnua_background")! as HTMLInputElement).checked,
        tamhue_slash: (document.getElementById("tamhue_slash")! as HTMLInputElement).checked,
        tamzo_color: (document.getElementById("tamzo_green")! as HTMLInputElement).checked ? "green" :
            (document.getElementById("tamzo_orange")! as HTMLInputElement).checked ? "orange" :
                (document.getElementById("tamzo_blue")! as HTMLInputElement).checked ? "blue" :
                    (document.getElementById("tamzo_white")! as HTMLInputElement).checked ? "white" : null
    });
