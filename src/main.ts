//! KAマスに駒を置いたときに、画像が0度回転したときと180度回転したときとで path の冒頭の `m ??? ???` がどうなるかを記録する表。 
//! これにより、駒の回転中心を計算する。
const GLOBAL_OFFSET_TABLE = {
    船: { 180: { x: 48.33, y: 103.2 }, 0: { x: 51.81, y: 90.38 } },
    兵: { 180: { x: 50.798, y: 102.73 }, 0: { x: 49.341, y: 90.84 } },
    弓: { 180: { x: 48.56, y: 102.49 }, 0: { x: 51.582, y: 91.08 }, },
    車: { 180: { x: 47.243, y: 103.02 }, 0: { x: 52.896, y: 90.55 } },
    馬: { 180: { x: 49.519, y: 103.59 }, 0: { x: 50.62, y: 89.98 } },
    虎: { 180: { x: 52.5, y: 102.64 }, 0: { x: 47.64, y: 90.94 } },
    筆: { 180: { x: 51.459, y: 103.86 }, 0: { x: 48.695, y: 89.81 } },
    巫: { 180: { x: 50.753, y: 103.06 }, 0: { x: 49.39, y: 90.51 } },
    将: { 180: { x: 51.46, y: 104.12 }, 0: { x: 48.68, y: 89.46 } },
    王: { 180: { x: 53.47, y: 103.59 }, 0: { x: 46.67, y: 89.99 } },
    皇: { 180: { x: 51.8, y: 103.76 }, 0: { x: 48.34, y: 89.82 } }
};
...
function piece(color: Color, profession: Profession, column: Column, row: Row, degree: Degree) {
    ...
    g.setAttributeNS('http://www.w3.org/2000/svg', 'transform', `translate(${LEFT * 17.55} ${TOP * 17.55}) rotate(${degree} ${(GLOBAL_OFFSET_TABLE[profession][0].x + GLOBAL_OFFSET_TABLE[profession][180].x) / 2
        } ${(GLOBAL_OFFSET_TABLE[profession][0].y + GLOBAL_OFFSET_TABLE[profession][180].y) / 2
        })`)
    g.appendChild(piece_path(color, profession)!); /* FIXME: ! */
    return g;
}
