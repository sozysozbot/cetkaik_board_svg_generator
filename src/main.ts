function piece_path(color: Color, profession: Profession, column: Column, row: Row, degree: Degree) {
    const TOP = {
...
type Color = "赤" | "黒";
type Degree = 0 | 180;
type Profession = "王" | "将" | "兵" | "弓" | "馬" | "車" | "巫" | "虎" | "筆" | "船";
...
function isDegree(degree: number): degree is Degree {
...
function render() {
    const input: Array<[Color, Profession, Column, Row, Degree]> = (document.getElementById("board_state")! as HTMLInputElement).value
        ...
        .flatMap(line => {
            ...
            if (!isDegree(degree_)) { msg.push(`invalid degree_ ${degree_}`) }
